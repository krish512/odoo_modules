...
class CrmLead(models.Model):
    ...
    def get_adset(self, lead):
        ...
        if not fb_adset:
            return ad_obj.create(
                {
                    "id_facebook_adset": lead["adset_id"],
    ...
    def get_campaign(self, lead):
        ...
        if not fb_camp:
            return campaign_obj.create({"id_facebook_campaign": lead["campaign_id"], "name": lead["campaign_name"]}).id
    ...
    def process_lead_field_data(self, lead):
        ...
        lead_data = dict(lead)
        lead_data.update([(ld["name"], ld["values"][0]) for ld in field_data if ld.get("name") and ld.get("values")])
        return lead_data
